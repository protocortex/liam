...
Every pull request is automatically checked for two hard requirements before review begins.

### Linked issue

Every PR must reference an issue in its description. Use one of GitHub's closing keywords:

```
Closes #N
```

This creates an audit trail connecting the change to its motivation. PRs without a linked issue are automatically closed by the quality gate and must be reopened after adding the reference.

### Human authorship

PRs must represent your own considered work. AI-assisted coding is fine, but:

- The PR description must be written by you, not an AI tool.
- Disclosures such as "Generated by Claude / ChatGPT / AI" in the title or body, or a `🤖` marker, will trigger automatic closure.
- You must be able to explain every change you propose and have tested it locally.

The motivation is signal quality: AI-generated PRs frequently have no test coverage, no linked issue, and no human who actually understands the change. Reopen after rewriting the description in your own words.
